...
class AddProduct extends React.Component {
  componentDidMount = () => {
    if (!this.props.actionState.initialization.storageGuid) {
      return this.redirect();
    }
    const selected = this.props.actionState.initialization.dataSource;
    let selectedArr = [];
    selected.map( (item, index) => selectedArr.push(item.storageMaterialGuid));
    this.setState({
      selected: selectedArr, 
      selectedRows: this.props.actionState.initialization.dataSource,
      allRows: this.props.actionState.initialization.dataSource
    });
  }
  state = {
    query: {
      searchLike: '',
      storageGuid: this.props.actionState.initialization.storageGuid
    },
    allRows: [],
    selectedRows: [], 
  ...
  add = () => {
    if (this.state.selected.length === 0) {
      ...
      const data = this.props.actionState.initialization.dataSource;
      rows.map( (item, index) => {
        if (data.length > 0) {
          data.map( (list, i) => {
      ...
      this.props.actions.createInitialization({
        dataSource: rows
      })
      this.redirect(true);
    }
  ...
  redirect = (status) => {
    hashHistory.push({
      pathname:'/storage/wareHouse/initialization'
    })
  }
  render () {
    return (
      <div>
        <Breadcrumb style={{fontSize: '1.1em',marginBottom:24}}>
           ...
           <Breadcrumb.Item><Link to={{pathname:'/storage/wareHouse/initialization'}}>初始化</Link></Breadcrumb.Item>
