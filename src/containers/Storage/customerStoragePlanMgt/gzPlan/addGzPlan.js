  ...
  save = () => {
      ...
      fetchData(storage.INSERTEDITPLAN,JSON.stringify(values),(data)=>{
        if(data.status){
          ...
          hashHistory.push({pathname:'/storage/customerStoragePlanMgt',query:{activeKey:'2'}});
        }
        else{
            ...
            <Button 
              ...
              onClick={() => {
                if (this.props.data.storageGuid) {
                  hashHistory.push({
                    pathname: '/storage/customerStoragePlanMgt/gzPlan/addGzMaterial'
                  })
                } else {
                  message.error('请选择库房!')
                }
...
class NewAddPh extends React.Component {
  ...
  onChange = (record, index, value) => {
    let { dataSource } = this.props.actionState.AddGzPlan;
    let total = 0;
    if (/^\d+$/.test(value)) {
  ...
  createAddGzPlan = (gzList) => {
    this.props.actions.createAddGzPlan(gzList);
  }
  ...
  render () {
    ...
    return (
        ...
        <Breadcrumb style={{fontSize: '1.1em',marginBottom:24}}>
          ...
          <Breadcrumb.Item>新建高值计划</Breadcrumb.Item>
        </Breadcrumb>
        <WrappedAddForm 
          cb={(gzList) => this.createAddGzPlan(gzList)}
          data={this.props.actionState.AddGzPlan}
        ...
        <Table 
          ...
          size={'small'}
          rowKey={'RN'}
          dataSource={ this.props.actionState.AddGzPlan.dataSource } 
          columns={columns} 
          scroll={{ x: '150%' }}
          pagination={false}
          footer={ this.props.actionState.AddGzPlan.dataSource.length === 0 ?
                  null : () => <span style={{fontSize: '1.5em'}}>总金额:
                                <a style={{color: '#f46e65'}}>
                                  {this.total(this.props.actionState.AddGzPlan.dataSource).toFixed(2)}
                                </a>
                              </span>}
        />
      </div>
