...
class AddPhMaterial extends React.Component {
  componentDidMount = () => {
    if (!this.props.actionState.AddPh.storageGuid) {
      return this.redirect();
    }
    const selected = this.props.actionState.AddPh.dataSource;
    let selectedArr = [];
    selected.map( (item, index) => selectedArr.push(item.storageMaterialGuid));
    this.setState({
      selected: selectedArr, 
      selectedRows: this.props.actionState.AddPh.dataSource,
      allRows: this.props.actionState.AddPh.dataSource
    });
  }
  state = {
    query: {
      searchLike: '',
      storageGuid: this.props.actionState.AddPh.storageGuid
    },
    allRows: [],
    selectedRows: [], 
  ...
  add = () => {
    if (this.state.selected.length === 0) {
      ...
      const data = this.props.actionState.AddPh.dataSource;
      rows.map( (item, index) => {
        if (data.length > 0) {
          data.map( (list, i) => {
      ...
      this.props.actions.createAddPh({
        dataSource: rows
      })
      this.redirect(true);
    }
  ...
  redirect = (status) => {
    hashHistory.push({
      pathname:  '/storage/customerStoragePlanMgt/phPlan/addPh'
    })
  }
  render () {
    return (
      <div>
         ...
         <Breadcrumb.Item><Link to={{pathname:'/storage/customerStoragePlanMgt'}}>计划管理</Link></Breadcrumb.Item>
          ...
          <Breadcrumb.Item><a onClick={this.redirect.bind(this, false)}>新建普耗计划</a></Breadcrumb.Item>
