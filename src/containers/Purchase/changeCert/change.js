...
import { productUrl,purchase } from 'api';
...
class ChangeCert extends React.Component {
  ...
  handleSumbit = () =>{
    ...
    const getData = this.props.location.state;
      fetchData(purchase.CONFIRMCHANGE,querystring.stringify({tenderStorageGuid:getData.tenderStorageGuid,newCertGuid:this.state.newCertGuid}),(data) => {
        if(data.status){
          hashHistory.push({pathname:'/purchase/changeCert/product',state:this.props.location.state});
    ...
    return (
      <div>
        <Breadcrumb style={{fontSize: '1.1em'}}>
          ...
          <Breadcrumb.Item><Link to={{pathname:'/purchase/changeCert/product',state:this.props.location.state}}>产品</Link></Breadcrumb.Item>
          <Breadcrumb.Item>{data.title}</Breadcrumb.Item>
        </Breadcrumb>
        <Row>
          <Col span={2} offset={20}>
            <Button type="primary" onClick={this.showModal}>确认变更</Button>
          </Col>
        </Row>
        <Row>
          <Col span={1}></Col>
          <Col span={22}>
            <table className="certTable">
              ...
              <tr>
                <td>证件图片:</td>
                <td>
                  {data.tfAccessoryFile===""||data.tfAccessoryFile===undefined?"无":<a href={pathConfig.YSYPATH+data.tfAccessoryFile} target="_blank">预览</a>}
                </td>
                <td>
              ...
              <tr>
                <td>附件:</td>
                <td>
                  {data.tfAccessory===null||data.tfAccessory===undefined?"无":<a href={pathConfig.YSYPATH+data.tfAccessory+"?isPreview=false"}>下载</a>}
                </td>
                <td>
                  {newData.tfAccessory===null||newData.tfAccessory===undefined?"无":<a href={pathConfig.YSYPATH+newData.tfAccessory+"?isPreview=false"}>下载</a>}
                </td>
              </tr>
        ...
        <Modal
          ...
          cancelText="取消"
        >
          <p>变更证件后产品将使用新证件的产品名称、品牌、生产商等信息。继续请<span style={{color:"#108ee9"}}>“确认”</span>，停止请<span style={{fontWeight:"bolder"}}>“取消”</span></p>
        </Modal>
      </div>
    );
  }
