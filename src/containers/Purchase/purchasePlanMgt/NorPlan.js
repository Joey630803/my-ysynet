...
class NorPlan extends React.Component{
		state = {
        query : {},
        selectedRowKeys:[],
        selectedRows:[],
    ...
    batchAll = () => {
  	...
  	//发送订单
    sendOrder = (record) => {
      const handData = [ record ];
      this.handleOrder(handData,'PLAN');
    }
     //发送所有已确认订单
    sendSureOrder = () =>{
      const selectedRows = this.state.selectedRows;
    ...
    showModal=()=>{
      ...
      this.refs.failReason.refs.input.value= "";
    }
    //取消
  	handleCancel=()=>{
      ...
      <div>
         ...
         <div>
          <SearchBox query={(query)=>this.queryHandler(query)} 
            defaultValue={query.bStorageGuid} 
            storageOption={this.state.storageOptions}
          ...
          <div>
            <Row>
              ...
              <Col span={16} style={{textAlign:'right'}}>
                ...
                <a href={purchase.EXPORTPLANLIST+"&"+querystring.stringify(query)}><Button type="primary" style={{marginRight:8}}>导出</Button></a>
              </Col>
            </Row>
            <FetchTable 
              style={{marginTop:10}}
              ...
              ref='table'
              url={purchase.SEARCHNORPLANLIST}
              rowKey={'planId'}
            ...
            >
            <h2>是否驳回?</h2>
            <Input style={{marginTop:'16px'}} placeholder='请输入驳回说明' ref='failReason' type="textarea" rows={4}/>
            </Modal>
          </div>
        }
        </div>
