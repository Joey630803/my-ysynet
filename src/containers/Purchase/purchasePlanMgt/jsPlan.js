...
import JsSearchForm from './JsSearchForm';
const confirm = Modal.confirm;
const Option = Select.Option
/** 挂载查询表单 */
const SearchBox = Form.create()(JsSearchForm);
...
class jsPlan extends React.Component{
		state = {
            query:{},
            selectedRowKeys:[],
            selectedRows:[],
    ...
    getActions = ( text , record )=>{
  	...
  	//详情
  	details = (record) => {
        this.redirect('/purchase/purchasePlanMgt/jsPlanShow', record)
  	}
  	//单条确认
    isSure = (record) => {
      let handData = [ record ];
    ...
    handleOrder = (record)=>{
      ...
      fetchData(purchase.SENDJSORDERBYSETTLE,querystring.stringify(values),(data)=>{
          if(data.status){
              this.refs.table.fetch();
              message.success("操作成功!")
          }else{
    ...
    batchAll = () => {
      ...
      confirm({
        ...
        onOk(){
          ...
          const values = {
            planType : 'SETTLE_PLAN',
            storageGuid : that.state.query.storageGuid,
            bStorageGuid : that.state.query.bStorageGuid,
            fstate: that.state.query.fstate,
            startDate : that.state.query.startDate,
            endDate : that.state.query.endDate,
            planNo: that.state.query.planNo
          }
    ...
    const exportHref = purchase.EXPORTJSPLANLIST+"&"+querystring.stringify(query)
		return (
    <div>
        {
        this.props.children || 
        <div>
			<SearchBox query={(query)=>this.queryHandler(query)} defaultValue={query.bStorageGuid}/>
            {
